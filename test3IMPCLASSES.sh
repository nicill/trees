...
do
  ...
  do
    ...
    do
        ...
        date
	if [[ $compute = 1 ]];then
	        CUDA_VISIBLE_DEVICES=$GPU;python $softDir/tree_detection.py -d $dataDir -e 20 -labFus $conversion -aug $a -dec $d -th $th -u2 True -imp1 $i1 -unimp1 $ui1 -imp2 $i2 -unimp2 $ui2 -ign2 $ig2
	fi
	if [[ $evaluate = 1 ]];then
